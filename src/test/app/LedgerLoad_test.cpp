...
class LedgerLoad_test : public beast::unit_test::suite
{
    ...
    testBadFiles(SetupData const& sd)
    {
        ...
        // file does not exist
            ...
            Env env(
                *this,
                envconfig(ledgerConfig, sd.dbPath, "badfile.json", StartUpType::LoadFile, std::nullopt),
                nullptr,
                beast::severities::kDisabled);
        });
        ...
        if (!BEAST_EXPECTS(!ec, ec.message()))
            ...
            Env env(
                *this,
                envconfig(ledgerConfig, sd.dbPath, ledgerFileCorrupt.string(), StartUpType::LoadFile, std::nullopt),
                nullptr,
                beast::severities::kDisabled);
        });
    }
