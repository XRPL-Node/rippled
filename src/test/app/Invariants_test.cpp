...
class Invariants_test : public BaseInvariants_test
{
    ...
    //                 ac.peek(keylet::account(pseudoId)); if
    //                 (!slePseudoAccount)
    //                     return false;
    ...
    //                 auto sleMPToken = ac.peek(keylet::mptoken(mptId,
    //                 pseudoId)); if (!sleMPToken)
    //                     return false;
    ...
    //         [&](Account const& A1, Account const& A2, Env& env) {
    //             Vault vault{env};
    //             auto [tx, _] = vault.create({.owner = A1, .asset =
    //             xrpIssue()}); env(tx); return true;
    //         });
    ...
    //         [&](Account const& A1, Account const& A2, ApplyContext& ac) {
    //             {
    //                 auto const keylet = keylet::vault(A1.id(),
    //                 ac.view().seq()); auto sleVault = ac.view().peek(keylet);
    //                 if (!sleVault)
    //                     return false;
    //                 ac.view().erase(sleVault);
    //             }
    //             {
    //                 auto const keylet = keylet::vault(A2.id(),
    //                 ac.view().seq()); auto sleVault = ac.view().peek(keylet);
    //                 if (!sleVault)
    //                     return false;
    //                 ac.view().erase(sleVault);
    ...
    //             if (!sleShares)
    //                 return false;
    //             // Note, such an "orphaned" update of MPT issuance attached
    //             to a
    //             // vault is invalid; ttVAULT_SET must also update Vault
    //             object. sleShares->setFieldH256(sfDomainID, uint256(13));
    ...
    //                     sample.assetsAvailable = (DROPS_PER_XRP *
    //                     -100).value(); sample.assetsTotal = (DROPS_PER_XRP *
    //                     -200).value(); sample.sharesTotal = -1;
    //                 }));
    //         },
    //         XRPAmount{},
    //         STTx{ttVAULT_SET, [](STObject& tx) {}},
    ...
    //             "created vault must be empty",
    //             "updated zero sized vault must have no assets available",
    //             "assets available must not be greater than assets
    //             outstanding", "create operation must not have updated a
    //             vault",
    //         },
    //         [&](Account const& A1, Account const& A2, ApplyContext& ac) {
    //             auto const keylet = keylet::vault(A1.id(), ac.view().seq());
    ...
    //         {
    //             "created vault must be empty",
    //             "loss unrealized must not exceed the difference between
    //             assets " "outstanding and available", "vault transaction must
    //             not change loss unrealized", "create operation must not have
    //             updated a vault",
    //         },
    //         [&](Account const& A1, Account const& A2, ApplyContext& ac) {
    //             auto const keylet = keylet::vault(A1.id(), ac.view().seq());
    ...
    //          amount", "deposit and assets outstanding must add up", "deposit
    //          and assets available must add up"},
    //         [&](Account const& A1, Account const& A2, ApplyContext& ac) {
    //             auto const keylet = keylet::vault(A1.id(), ac.view().seq());
    ...
    //             balance auto sleA3 =
    //             ac.view().peek(keylet::account(A3.id())); if (!sleA3)
    //                 return false;
    //             (*sleA3)[sfBalance] = *(*sleA3)[sfBalance] - 10;
    //             ac.view().update(sleA3);
    ...
    //          amount", "deposit must not change vault balance by more than
    //          deposited " "amount"},
    //         [&](Account const& A1, Account const& A2, ApplyContext& ac) {
    //             auto const keylet = keylet::vault(A1.id(), ac.view().seq());
    //             return adjust(
