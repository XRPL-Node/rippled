...
SQLiteDatabase::getOldestAccountTxs(AccountTxOptions const& options)
{
    ...
    {
        auto db = checkoutTransaction();
        return detail::getOldestAccountTxs(*db, registry_.getApp(), ledgerMaster, options, j_).first;
    }
...
SQLiteDatabase::getNewestAccountTxs(AccountTxOptions const& options)
{
    ...
    {
        auto db = checkoutTransaction();
        return detail::getNewestAccountTxs(*db, registry_.getApp(), ledgerMaster, options, j_).first;
    }
...
SQLiteDatabase::oldestAccountTxPage(AccountTxPageOptions const& options)
{
    ...
    auto onUnsavedLedger = std::bind(saveLedgerAsync, std::ref(registry_.getApp()), std::placeholders::_1);
    AccountTxs ret;
...
SQLiteDatabase::SQLiteDatabase(ServiceRegistry& registry, Config const& config, JobQueue& jobQueue)
    : registry_(registry), useTxTables_(config.useTxTables()), j_(registry.getJournal("SQLiteDatabase"))
{
